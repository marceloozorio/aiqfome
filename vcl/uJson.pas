...
Type
  ...
  public
    ...
    *)
    ...
    function put (key : string; value : double; currencyDecimals: byte): TJSONObject; overload;
...
begin
  c := nextClean();
        ...
        if ( ((b >= '0') and (b <= '9')) or (b = '.')
            ...
            if (b = '0') and (TryStrToInt(s,sInteger)) then begin
            ...
            if TryStrToInt(s,sInteger) then begin
              try
...
(**
     ...
     * @return this.
     *)
function TJSONObject.put (key : string; value : double; currencyDecimals: byte): TJSONObject;
var doubleObj: _Double;
begin
   doubleObj:=_Double.create(value);
   doubleObj.FcurrencyDecimals:=currencyDecimals;
   put(key, doubleObj);
   result := self;
end;
...
begin
  ...
  FcurrencyDecimals:=formatSettings.CurrencyDecimals;
  fvalue := StrToFloat (s, formatSettings);
end;
...
var decimals: string;
begin
  decimals:=padLeft('', '0', FcurrencyDecimals);
  result := StringReplace(formatFloat('######0.' + decimals,fvalue),',','.',[rfReplaceAll]);
end;
...
begin
  result := self is TZAbstractObject;
end;
